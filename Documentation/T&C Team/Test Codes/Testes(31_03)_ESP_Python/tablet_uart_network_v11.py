...
def parse_and_handle_packet(packet, emit_logs=True):
    ...
    if emit_logs:
        print(f"Recebi {tamanho} bytes cifrados: {packet.hex()}")
        print(f"Payload limpa: {decrypted_packet.hex()}")
    ...
    if tamanho == 15:
        ...
        if emit_logs:
            print("=========================================")
            print(f"TRAMA COMPLETA | Net={dados['net_id']} | Colete={dados['vest_id']}")
            print(f"BPM:   {dados['bpm']}")
            print(f"SpO2:  {dados['spo2']}%" if dados["spo2"] != 0 else "SpO2:  ERRO/HIPOXIA (<70%)")
            print(f"Temp:  {dados['temperature']:.2f} C")
            print(f"Alt:   {dados['altitude']} m")
            print(f"GPS:   {dados['lat']:.7f}, {dados['lon']:.7f}")
            print("=========================================\n")
        return dados
    ...
    if tamanho == 7:
        ...
        if not dados:
            if emit_logs:
                print(f"Delta recebido para colete {dados_delta['vest_id']}, mas ainda sem trama completa base.")
            return None
        mark_vest_seen(dados["vest_id"], "DELTA_GPS")
        if emit_logs:
            print("=========================================")
            print(f"TRAMA DELTA | Net={dados['net_id']} | Colete={dados['vest_id']}")
            print(f"GPS:   {dados['lat']:.7f}, {dados['lon']:.7f}")
            print(f"Alt:   {dados['altitude']} m (delta {dados['alt_delta']:+d})")
            print("=========================================\n")
        return dados
