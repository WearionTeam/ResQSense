...
def slot_start_for_vest(vest_id):
    ...
    return tdma_epoch + (vest_id - 1) * TDMA_SLOT_SECONDS
...
def create_network_alive_packet(net_id):
    ...
    return bytes([
        ...
        make_control_byte(OPCODE_NETWORK_ALIVE, FUNCT_NETWORK_ALIVE_BEACON),
    ])
...
def create_team_hop_packet(net_id, data_ch, ctrl_ch):
    ...
    return bytes([
        make_header(net_id, 0),
        make_control_byte(OPCODE_NETWORK_ALIVE, FUNCT_NETWORK_ALIVE_TEAM_HOP),
...
def unpack_team_hop_frame(decrypted_data):
    ...
    vest_id = decrypted_data[0] & 0x1F
    opcode, funct = split_control_byte(decrypted_data[1])
    if opcode != OPCODE_NETWORK_ALIVE or funct != FUNCT_NETWORK_ALIVE_TEAM_HOP:
        return None
    ...
    return {
        ...
        "opcode": opcode,
        "funct": funct,
        "data_ch": (decrypted_data[2] >> 5) & 0x07,
        "ctrl_ch": (decrypted_data[2] >> 2) & 0x07,
    }
...
def unpack_sync_frame(decrypted_data):
    ...
    opcode = (decrypted_data[1] >> 4) & 0x0F
    ...
    return {
        ...
        "vest_id": decrypted_data[0] & 0x1F,
        "opcode": opcode,
        "target_net_id": decrypted_data[1] & 0x07,
        "mac_address": struct.unpack(">I", decrypted_data[2:6])[0],
        "data_ch": (decrypted_data[6] >> 5) & 0x07,
        "ctrl_ch": (decrypted_data[6] >> 1) & 0x07,
    }
...
def wait_for_ack(expected_vest_id, expected_funct, timeout=8.0):
    ...
    validation_log("WAIT", f"Waiting for ACK | Vest={expected_vest_id} | Funct=0x{expected_funct:02X} | Timeout={timeout:.2f}s")
    while time.time() < end_time:
        ...
        if control and control["opcode"] == OPCODE_ACK:
            validation_log("ACK", f"ACK received | Vest={control['vest_id']} | Funct=0x{control['funct']:02X}")
            if control["vest_id"] == expected_vest_id and control["funct"] == expected_funct:
                validation_log("ACK", "Expected ACK confirmed.")
                return True
    validation_log("ACK", "ACK timeout expired.")
    return False
...
def send_sync_process(new_vest_id, mac_address_int, target_net_id=DEFAULT_NET_ID,
    ...
    validation_log("SYNC", f"Preparing SYNC packet for Vest={new_vest_id} | Net={target_net_id} | MAC=0x{mac_address_int:08X}")
    validation_log("SYNC", f"Encrypted packet: {format_hex_bytes(encrypted_packet)}")
    ...
    with sync_lock:
        for attempt, wait_time in enumerate(wait_intervals, start=1):
            ...
            if wait_for_ack(new_vest_id, FUNCT_ACK_SYNC, timeout=wait_time):
                ...
                validation_log("SYNC", f"Colete {new_vest_id} entered the network successfully.")
                print(f"SYNC bem sucedido para colete {new_vest_id}\n")
                return True

    validation_log("SYNC", f"Colete {new_vest_id} did not confirm the SYNC process.")
...
def send_unsync_process(vest_id, funct=FUNCT_UNSYNC_MANUAL, timeout=3.0):
    ...
    validation_log("UNSYNC", f"Preparing UNSYNC packet for Vest={vest_id}")
    validation_log("UNSYNC", f"Encrypted packet: {format_hex_bytes(encrypted)}")
    ...
    hardware_set_role_control()
    validation_log("TX", f"Sending UNSYNC to Vest={vest_id}")
    hardware_send_packet(encrypted)
    if wait_for_ack(vest_id, FUNCT_ACK_SYNC, timeout=timeout):
        remove_vest(vest_id)
        validation_log("UNSYNC", f"Colete {vest_id} removed from the network.")
        print(f"UNSYNC bem sucedido para colete {vest_id}\n")
        return True

    validation_log("UNSYNC", f"Colete {vest_id} did not confirm the UNSYNC process.")
...
def parse_and_handle_packet(packet, emit_logs=True):
    ...
    if tamanho == 7:
        if emit_logs:
            ...
            print(f"[PARSER] Packet length detected: {tamanho} bytes")
            print(f"[PARSER] Decrypted payload: {format_hex_bytes(decrypted_packet)}")
            print("[PARSER] Decoding 7-byte Packet...")
        sync = unpack_sync_frame(decrypted_packet)
        if sync:
            register_vest(sync["vest_id"], mac_address=sync["mac_address"], net_id=sync["target_net_id"])
            if emit_logs:
                print(
                    f"[DATA] Type: Sync Packet | Vest={sync['vest_id']} | "
                    f"Target Net={sync['target_net_id']} | MAC=0x{sync['mac_address']:08X}"
                )
                print(f"[DATA] Data CH: {sync['data_ch']} | Ctrl CH: {sync['ctrl_ch']}\n")
            return sync
        if emit_logs:
    ...
    if tamanho == 4:
        if emit_logs:
            ...
            print(f"[PARSER] Packet length detected: {tamanho} bytes")
            print(f"[PARSER] Decrypted payload: {format_hex_bytes(decrypted_packet)}")
            print("[PARSER] Decoding Biometric Alert Packet...")
        alert = unpack_bio_alert_frame(decrypted_packet)
        if alert and emit_logs:
            print(
                f"[DATA] Type: Biometric Alert | Net={alert['net_id']} | Colete={alert['vest_id']} | "
    ...
    if tamanho == 3:
        if emit_logs:
            ...
            print(f"[PARSER] Packet length detected: {tamanho} bytes")
            print(f"[PARSER] Decrypted payload: {format_hex_bytes(decrypted_packet)}")
            print("[PARSER] Decoding Team Hop Packet...")
        hop = unpack_team_hop_frame(decrypted_packet)
        if hop and emit_logs:
            print(
                f"[DATA] Type: Team Hop | Net={hop['net_id']} | Data CH={hop['data_ch']} | Ctrl CH={hop['ctrl_ch']}"
            )
            print()
        return hop
    ...
    if tamanho == 2:
        if emit_logs:
            ...
            print(f"[PARSER] Packet length detected: {tamanho} bytes")
            print(f"[PARSER] Decrypted payload: {format_hex_bytes(decrypted_packet)}")
            print("[PARSER] Decoding Control Packet...")
        control = unpack_control_frame(decrypted_packet)
        if control and emit_logs:
            print(
                f"[DATA] Type: Control | Net={control['net_id']} | Colete={control['vest_id']} | "
...
def network_alive_thread():
    ...
    while running:
        try:
            packet = encrypt_aes_ctr(create_network_alive_packet(DEFAULT_NET_ID))
            hardware_set_role_control()
            validation_log("TX", "Sending periodic Network Alive beacon.")
            validation_log("NETWORK", f"Encrypted packet: {format_hex_bytes(packet)}")
            hardware_send_packet(packet)
            time.sleep(60)
        except Exception as e:
            print(f"ERRO NETWORK ALIVE: {e}")
            time.sleep(5)
...
if __name__ == "__main__":
    ...
    # Thread de envio periodico de Network Alive.
    alive_thread = threading.Thread(target=network_alive_thread, daemon=True)
    alive_thread.start()
    ...
    # Teste anterior:
    # validar o SYNC e confirmar o registo do colete na tabela TDMA.
    # time.sleep(1)
    # send_sync_process(3, 0x11223344)
    # print_tdma_table()

    # Teste atual:
    # 1. fazer SYNC
    # 2. deixar tempo para receber telemetria
    # 3. validar o UNSYNC
    time.sleep(1)
    send_sync_process(3, 0x11223344)
    print_tdma_table()
    time.sleep(5)
    send_unsync_process(3)
    print_tdma_table()
