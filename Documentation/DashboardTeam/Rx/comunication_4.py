...
if MODO_HARDWARE == "LORA":
    ...
    try:
        # definir a porta do cabo. Exemplo Windows: "COM3" | Linux/Raspberry: "/dev/serial0"
        PORTA_UART = "COM3" 
...
def wait_for_ack(expected_vest_id, timeout):
    ...
    while (time.time() - start_time) < timeout:
        ...
        if packet is not None and len(packet) == 2:
            ...
            if received_vest_id == expected_vest_id and opcode == 0b1001 and funct == 0b0011:
                return True
    return False
...
def listen_for_incoming_data():
    if packet is not None:
        print(f"Recebi {len(packet)} bytes: {packet.hex()}")
    ...
    if packet is not None:
        ...
        if tamanho == 15:
            ...
            flags = decrypted_packet[1] # o Byte 1 fica guardado caso seja preciso ler os alertas
            
            # Lemos os 5 bytes restantes (Bytes 2 a 6)
            # '>HH b' significa: Big-Endian, 2 Unsigned Shorts (Lat/Lon) e 1 Signed Byte (Altitude)
            lat_min_novo, lon_min_novo, alt_delta = struct.unpack('>HH b', decrypted_packet[2:7])
            ...
            if vest_id in memoria_gps:
                ...
                limite_superior = 9000000 
                limite_inferior = 1000000 
                ...
                if estado['lat_min'] > limite_superior and lat_min_novo < limite_inferior:
                    ...
                    estado['lat_graus'] -= 1
                estado['lat_min'] = lat_min_novo 

                # Lógica de Rollover da Longitude
                ...
                print(f"[GPS DELTA] Colete {vest_id} -> Lat: {estado['lat_graus']}.{estado['lat_min']} | Alt Variação: {alt_delta}m")
            ...
            # 3. ALERTA BIOMÉTRICO (BPM, Temperatura, SpO2)
            vest_id = decrypted_packet[0] & 0x1F
            opcode = (decrypted_packet[1] >> 4) & 0x0F
            funct = decrypted_packet[1] & 0x0F
            
            # Lê os 2 bytes finais (16 bits) para saber o valor exato que o sensor mediu
            valor_exato = struct.unpack('>H', decrypted_packet[2:4])[0]
            
            print(f"!!! ALERTA BIOMÉTRICO: Colete {vest_id} | Valor Registado: {valor_exato} !!!")
            ...
            # 4. TRAMAS DE CONTROLO (ACKs ou QUICK ALERTS)
            vest_id = decrypted_packet[0] & 0x1F
            opcode = (decrypted_packet[1] >> 4) & 0x0F
            funct = decrypted_packet[1] & 0x0F
            
            if opcode == 0b1001: 
                # Se for OpCode 9, é o ACK normal
                print(f"[ACK] Colete {vest_id} confirmou a receção da mensagem.")
                return {"tipo": "ACK", "vest_id": vest_id, "status": "RECEBIDO"}
                
            elif opcode == 0b1111:
                # Se for OpCode 15, é um QUICK ALERT (SOS ou Man Down)
                if funct == 0b1111:
                    print(f"!!! EMERGÊNCIA (SOS): O Colete {vest_id} acionou o botão de Pânico !!!")
                elif funct == 0b1100:
                    print(f"!!! EMERGÊNCIA (MAN DOWN): O Colete {vest_id} detetou uma queda !!!")
