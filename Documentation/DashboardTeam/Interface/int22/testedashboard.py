...
class ResQSenseApp:
    def __init__(self, root):
        ...
        try:
            self.signal_loss_timeout_sec = float(os.getenv("RESQSENSE_SIGNAL_LOSS_TIMEOUT", "15"))
        except ValueError:
            self.signal_loss_timeout_sec = 15.0
        self.force_simulation_mode = os.getenv("RESQSENSE_FORCE_SIMULATION", "1").strip() != "0"
    ...
    def start_live_data_receiver(self):
        ...
        try:
            ...
            self.telemetry_receiver = None
            self.live_data_enabled = False
            print(f"[UART] Falha ao iniciar rececao ({exc}). Mantida simulacao.")
    ...
    def ensure_firefighter_for_vest(self, vest_id):
        ...
        for firefighter in self.firefighters:
            if firefighter.get("is_leader", False):
                leader_vest = str(firefighter.get("vest_id", "")).strip()
                if leader_vest and leader_vest == vest_key:
                    return firefighter
                continue
            if str(firefighter["id"]) == vest_key:
                return firefighter
        current_ops = [f for f in self.firefighters if not f.get("is_leader", False)]
        if len(current_ops) >= self.max_operationals:
            return None

        self.add_firefighter(vest_key, f"Colete {vest_key}")
        for firefighter in self.firefighters:
            if firefighter.get("is_leader", False):
                continue
            if str(firefighter["id"]) == vest_key:
    ...
    def update_radar_position_from_gps(self, firefighter, latitude, longitude):
        if self.radar_origin is None:
            self.radar_origin = (latitude, longitude)

        origin_lat, origin_lon = self.radar_origin
        ...
        # Bearing stored in radians from North, clockwise.
        firefighter["angle"] = math.atan2(delta_x, delta_y)
        firefighter["dist"] = max(0.05, min(0.95, distance_m / self.radar_max_distance_m))
    ...
    def apply_incoming_packet(self, packet):
        if packet.get("tipo") == "PORT_STATUS":
            ...
            print(f"[UART] Porta {port}: {status}")
            return
        ...
        if packet_type in ("COMPLETA", "DELTA_GPS"):
            ...
            if flags.get("vital") or is_critical:
    ...
    def update_simulation_cycle(self):
        ...
        if self.firefighters:
            for ff in self.firefighters:
                ...
                if self.live_data_enabled:
                    if is_leader and not self.leader_vest_id:
                        ff["status"] = "leader"
                        ff["active_warnings"] = []
                ...
                if should_persist_snapshot and not ff.get("signal_lost", False):
                    distance_m = self.normalized_distance_to_meters(ff.get("dist", 0.0))
    ...
    def add_leader_to_team(self, name):
        leader_vest_id = self.leader_vest_id if self.leader_vest_id else "LDR"
        leader_logic = Operator(id=0, name=name, vest_id=leader_vest_id)
        leader_dict = {
            "id": "C",
            "name": name,
    ...
    def show_next_man_down_popup(self):
        ...
        popup = tk.Toplevel(self.root)
        popup.title(self.t("critical_alert_title"))
        ...
        tk.Label(popup, text=self.t("man_down"), font=("Arial", 22, "bold"), bg=self.alert_color, fg="white").pack(pady=(0, 15))
        tk.Label(popup, text=self.t("is_down", name=alert_data["name"]), font=("Arial", 20, "bold"), bg=self.alert_color, fg="white").pack()
        tk.Label(
            popup,
            text=self.t("reason", reason=alert_data["reason"]),
            font=("Arial", 13, "bold"),
            bg=self.alert_color,
            fg="white",
            wraplength=320,
    ...
    def build_firefighter_details_text(self, firefighter):
        ...
        return (
            ...
            f"{self.t('distance')}: {self.normalized_distance_to_meters(firefighter.get('dist', 0.0)):.0f} m\n"
    ...
    def draw_radar(self, event=None):
        ...
        # Historico de warnings (ultimos 4 registos)
        entries = self.warning_history[-4:]
        list_start_y = warnings_top_y + 12
        row_height = 34
        left_col_x = 20
        left_col_w = 180
        right_col_x = left_col_x + left_col_w + 24
